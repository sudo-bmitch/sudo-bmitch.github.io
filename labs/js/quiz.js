...
$(() => {
  ...
  $newQuiz.find(".finish-quiz").click(() => {
    ...
    if (userId && typeof badgeId !== 'undefined') {
      fetch("/api/user/" + userId + "/badges" , {method: 'POST', body:"badgeId=" + badgeId, headers: {'Content-type': 'application/x-www-form-urlencoded'}}).then(function(res) {
        if (res.status == 200) {
